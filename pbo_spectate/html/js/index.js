$(function() {
    ...
    $('.users').on('click', '.spec', function() {
        ...
        if (target == player) {
            ...
            $('.spectate').fadeOut();
            $.post('http://pbo_spectate/select', JSON.stringify({
                id: target
            }));
        }
    ...
    $('.users').on('click', '.kick', function() {
        ...
        if (reason == null || reason == "") {
            ...
            $('.spectate').fadeOut();
            $.post('http://pbo_spectate/kick', JSON.stringify({
                id: target,
                reason: reason
            }));
        }
    ...
    $('.header').on('click', '#close', function() {
        $('.spectate').fadeOut();
        $.post('http://pbo_spectate/quit');
    });

    $('.header').on('click', '#minimize', function() {
        $('.spectate').fadeOut();
        $.post('http://pbo_spectate/quit');
    });
    ...
    $(document).keyup(function(e) {
        if (e.keyCode == 27) {
            $('.spectate').fadeOut();
            $.post('http://pbo_spectate/close');
            $.post('http://pbo_spectate/minimize');
        }
    })
